...
template <typename ImageDataType>
void copyToMatBGR_RowMaj(ImageDataType *src, ImageDataType *dst, int numRowsInBlock, int numCols) // removed channels from here
{
...
void cArrayToMat_RowMaj(const ImageDataType *in, int numRows, int numCols, bool isRGB, cv::Mat &out)
{
	...
	else
	{
		...
		for (int t = 1; t <= NUM_THREADS; t++)
		{
			...
			workers.push_back(thread(copyToMatBGR_RowMaj<ImageDataType>, &imgData[startRowIdx*nChannels], &dst[startRowIdx*nChannels], numRowsInBlock, numCols));
			startRowIdx = numRowsInBlock;
			numRowsInBlock = startRowIdx + blocks;
		}
#else
     	copyToMatBGR_RowMaj<ImageDataType>(imgData, dst, numRows, numCols);
#endif
	}
